...
// ───────────────────────── OPERATIONS ─────────────────────────
server.tool("baton_task_create", "검증 가능한 작업 그래프에 task를 만든다. depends_on은 선행 task 목록이다.",
  { api_key: z.string(), title: z.string(), room_id: z.string().optional(), assignee: z.string().optional(), depends_on: z.array(z.string()).optional() }, wrap((a) => core.taskCreate(a)));
server.tool("baton_task_link", "두 task를 blocks/relates/supersedes edge로 연결한다. blocks 순환은 거부한다.",
  { api_key: z.string(), from_task_id: z.string(), to_task_id: z.string(), edge_type: z.enum(["blocks","relates","supersedes"]).optional() }, wrap((a) => core.taskLink(a)));
server.tool("baton_task_update", "task 상태를 todo/running/blocked/done/cancelled 중 하나로 변경한다.",
  { api_key: z.string(), task_id: z.string(), status: z.enum(["todo","running","blocked","done","cancelled"]) }, wrap((a) => core.taskUpdate(a)));
server.tool("baton_task_graph", "내 task DAG와 edge를 조회한다.", { api_key: z.string() }, wrap((a) => core.taskGraph(a)));
server.tool("baton_git_record", "Git commit·diff digest·test 결과를 task에 결속한다. 서버는 저장소 credential을 받지 않는다.",
  { api_key: z.string(), task_id: z.string().optional(), repository: z.string(), commit_sha: z.string(), branch: z.string().optional(), diff_sha256: z.string().optional(), test_command: z.string().optional(), test_exit_code: z.number().int().optional(), artifact_refs: z.array(z.string()).optional() }, wrap((a) => core.gitRecord(a)));
server.tool("baton_git_evidence", "내 Git evidence 원장을 조회한다.", { api_key: z.string(), task_id: z.string().optional() }, wrap((a) => core.gitEvidence(a)));
...
if (process.env.BATON_HTTP === "1") {
  ...
  app.post("/api/task/create", rlWrite, api((b) => core.taskCreate(b)));
  app.post("/api/task/link", rlWrite, api((b) => core.taskLink(b)));
  app.post("/api/task/update", rlWrite, api((b) => core.taskUpdate(b)));
  app.post("/api/task/graph", rlRead, api((b) => core.taskGraph(b)));
  app.post("/api/git/record", rlWrite, api((b) => core.gitRecord(b)));
